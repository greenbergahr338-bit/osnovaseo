/* =========================
    ...
    function buildRefUrl(src){
      ...
      u.searchParams.set("sub4", (location && location.pathname) ? location.pathname : "");
      return u.toString();
    }
...
document.addEventListener('click', async (e) => {
  const btn = e.target.closest('[data-copy-promo]');
  if (!btn) return;
  const code = btn.getAttribute('data-copy-promo') || '';
  const hint = btn.parentElement ? btn.parentElement.querySelector('.promo-hint') : null;
  try {
    ...
    if (hint) hint.textContent = 'Не удалось скопировать';
  }
});
